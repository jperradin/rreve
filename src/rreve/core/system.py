...
class System:
    ...
    def load_frame(self, frame_index: int) -> bool:
        ...
        if not (
            start_frame
            <= frame_index
            <= (end_frame if end_frame != -1 else float("inf"))
        ):
            print(
                f"Frame index {frame_index} is out of range specified in settings ({start_frame}-{end_frame})."
            )
            return False
    ...
    def iter_frames(self) -> Generator[Frame, None, None]:
        ...
        if hasattr(self.reader, "frame_indices") and self.reader.frame_indices:
            for frame_id in range(
                start_frame,
                min(
                    end_frame + 1 if end_frame != -1 else float("inf"),
                    len(self.reader.frame_indices),
                ),
            ):
                try:
            ...
            for frame_id in range(
                start_frame, end_frame + 1 if end_frame != -1 else float("inf")
            ):
    ...
    def __next__(self) -> Frame:
        ...
        if self._current_frame_index is None:  # First call to next
            self._current_frame_index = self.settings.range_of_frames[
                0
        ...
        if (
            end_frame != -1 and self._current_frame_index > end_frame
        ):  # check if end frame is reach
            raise StopIteration
